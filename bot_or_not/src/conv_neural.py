...
def test_cnn(dataset, embedding, model, device):
    ...
    for batch in tqdm(loader):
        ...
        batch_emb = model(embedding(batch))
        batch_cos_dist = 0
        ...
        for i in range(batch_emb.shape[0]):
            for j in range(batch_emb.shape[0]):
                ...
                batch_cos_dist += cos_sim

        batch_cos_dist = batch_cos_dist/(batch_emb.shape[0] ** 2)
        total_cos_dist += batch_cos_dist
    
    return total_cos_dist/len(loader)
...
class test_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        return self.sentence_ids[index]
...
def test_collate(batch):
    ...
    sentences_out = torch.tensor(sentences_out, dtype=torch.int64)
